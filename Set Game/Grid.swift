...
struct Grid
{
    ...
    var margin: CGFloat = 0 { didSet { recalculate() } }
    ...
    private mutating func updateCellFrames(to cellSize: CGSize) {
        ...
        if cellCount > 0 {
            for _ in 0..<cellCount {
               cellFrames.append(CGRect(origin: CGPoint(x: origin.x + cellSize.width * margin, y: origin.y + cellSize.height * margin),
                                        size: CGSize(width: cellSize.width - cellSize.width * margin * 2, height: cellSize.height - cellSize.height * margin * 2)))
